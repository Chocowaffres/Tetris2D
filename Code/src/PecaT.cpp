...
// Cor da peça
std::vector<GLfloat> PecaT::g_color_buffer_data = {
		//T
		0.85f,  0.45f,  0.85f,
		0.85f,  0.45f,  0.85f,
		0.85f,  0.45f,  0.85f,
...
bool PecaT::atualizaMatriz() {
	switch (iNumberRotate % 4) {
		...
		case 1: return preencheMatriz(xPosE, yPos, 0, trunc(iPieceWidth / 2));
		case 2: return preencheMatriz(xPosE, yPos, trunc(iPieceWidth / 2), iPieceHeight - 1);
		case 3: return preencheMatriz(xPosE, yPos, iPieceWidth - 1, trunc(iPieceWidth / 2));
	}
}
...
bool PecaT::avaliaColisao() {
	...
	switch (iNumberRotate % 4) {
		...
		case 1:
			...
			break;
		case 2:
			if ((gameGrid[xPosE][yPos] == 1) || (gameGrid[xPosE + 1][yPos - 1] == 1) || (gameGrid[xPosE + 2][yPos] == 1)){
				return true;
			}
			break;
		case 3:
